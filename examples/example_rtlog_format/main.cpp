...
int main(int argc, const char **argv)
{
    ...
    LOG(info).format("{}", std::vector<int>{10, 20, 30});
    // Output: [10, 20, 30]

    LOG(info).format("{::#x}", std::vector<int>{10, 20, 30});
    // Output: [0xa, 0x14, 0x1e]

    LOG(info).format("{}", std::vector<char>{'h', 'e', 'l', 'l', 'o'});
    // Output: ['h', 'e', 'l', 'l', 'o']

    LOG(info).format("{:n}", std::vector<char>{'h', 'e', 'l', 'l', 'o'});
    // Output: 'h', 'e', 'l', 'l', 'o'
    ...
    LOG(info).format("{::d}", std::vector<char>{'h', 'e', 'l', 'l', 'o'});
    // Output: [104, 101, 108, 108, 111]

    uint16_t testVal = 0x12;
    auto ErrCodeName = [](uint16_t input) -> const char * {
        return "ERROR_TEST";
    };

    LOG(err) << "[" << __func__ << "] 0x"                        \
             << std::hex << std::setw(4) << std::setfill('0') << testVal \
             << std::dec << " " << ErrCodeName(testVal)                  \
             << " | " << "this is RtLog test code";  
